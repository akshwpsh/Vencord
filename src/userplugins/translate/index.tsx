...
import { stopPlugin, plugins as allPlugins } from "@api/PluginManager";
...
function getMessageContent(message: Message) {
    ...
    // There is no sensible way to fully flatten nested snapshots here.
...
function getTimestamp(message: Message) {
    ...
    return typeof ts?.valueOf === "function"
        ? ts.valueOf()
        : new Date(ts).getTime();
}
...
function getMessagesInGroup(channelId: string, messageId: string, authorId: string): Message[] {
    ...
    for (let i = currentIndex - 1; i >= 0; i--) {
        ...
        const timeDiff = Math.abs(getTimestamp(group[0]) - getTimestamp(msg));
    ...
    for (let i = currentIndex + 1; i < allMessages.length; i++) {
        ...
        const timeDiff = Math.abs(getTimestamp(msg) - getTimestamp(group[group.length - 1]));
        if (timeDiff > 7 * 60 * 1000) break;
...
export default definePlugin({
    name: "Gusal_Translate",
    description: "Google Translate, Gemini, DeepL로 메시지를 번역합니다",
    authors: [Devs.Ven, Devs.AshtonMemer, ...LocalDevs],
    settings,
    contextMenus: {
    ...
    start() {
        const builtInTranslate = allPlugins.Translate;
        if (!builtInTranslate || Settings.plugins.Translate?.enabled !== true) return;

        Settings.plugins.Translate.enabled = false;
        stopPlugin(builtInTranslate);
        showToast("내장 Translate를 비활성화하고 Gusal_Translate를 사용합니다.", Toasts.Type.MESSAGE);
    },
